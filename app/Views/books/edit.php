    ...
    <div class="flex items-center w-6/12 max-w-screen-xl mb-8">
        <div class="flex flex-col w-full">
            ...
            <div class="text-sm flex flex-col justify-center gap-2">
                ...
                <div class="my-2"></div>
                <div class="relative">
                    <input type="text" id="floating_outlined" class="border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('author', session('validation_errors')) && session('validation_errors')['author'] ? 'red-600' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('author', session('validation_errors')) && session('validation_errors')['author'] ? 'red' : 'blue' ?>-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="..." autofocus name="author" value="<?= (old('author')) ? old('author') : $book['author'] ?>" />
                    <label for="author" class="absolute text-[#434C5E] dark:text-[#434C5E] duration-300 transform -translate-y-4 scale-90 top-2 z-10 origin-[0] bg-[#E5E9F0] dark:bg-[#E5E9F0] px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-focus:dark:text-<?= session('validation_errors') && array_key_exists('author', session('validation_errors')) && session('validation_errors')['author'] ? 'red' : 'blue' ?> peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-2 peer-focus:scale-90 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">Author</label>
                </div>
                <div class="text-red-600"><?php
                ...
                <div class="my-2"></div>
                <div class="relative">
                    <textarea type="text" id="floating_outlined" class="h-32 border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('synopsis', session('validation_errors')) && session('validation_errors')['synopsis'] ? 'red-600' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('synopsis', session('validation_errors')) && session('validation_errors')['synopsis'] ? 'red-600' : 'blue-500' ?> focus:outline-none focus:ring-0 focus:border-blue-600 peer" autofocus name="synopsis"><?= (old('synopsis')) ? old('synopsis') : $book['synopsis'] ?></textarea>
                ...
                <select id="category_id" name="category_id" class="bg-[#E5E9F0] border border-[#434C5E] text-[#434C5E] text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-[#E5E9F0] dark:border<?= session('validation_errors') && array_key_exists('category_id', session('validation_errors')) && session('validation_errors')['synopsis'] ? 'red-600' : '[#434C5E]' ?> dark:text-[#434C5E] dark:focus:ring-blue-500 dark:focus:border-blue-500">
                ...
                <div class="my-2"></div>
                <div class="relative">
                    <input type="text" id="floating_outlined" class="border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('price', session('validation_errors')) && session('validation_errors')['price'] ? 'red-600' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('price', session('validation_errors')) && session('validation_errors')['price'] ? 'red-600' : 'blue-500' ?> focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" autofocus name="price" value="<?= (old('price')) ? old('price') : $book['price'] ?>"></input>
                ...
                <div class="my-2"></div>
                <div class="relative">
                    <input type="text" id="floating_outlined" class="border block px-4 pb-3 pt-4 w-full text-sm text-gray-900 bg-transparent rounded-lg border-1 border-gray-300 appearance-none dark:text-[#434C5E] dark:border-<?= session('validation_errors') && array_key_exists('stock', session('validation_errors')) && session('validation_errors')['stock'] ? 'red-600' : '[#434C5E]' ?> dark:focus:border-<?= session('validation_errors') && array_key_exists('stock', session('validation_errors')) && session('validation_errors')['stock'] ? 'red-600' : 'blue-500' ?> focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder="" autofocus name="stock" value="<?= (old('stock')) ? old('stock') : $book['stock'] ?>"></input>
